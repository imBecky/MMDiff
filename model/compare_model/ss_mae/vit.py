...
from .layers.patch_embd import PatchEmbed_spa, PatchEmbed_chan, PositionEmbed
from .layers.mlp import Mlp, Mlp_wo_gate
from .layers.drop import DropPath
from .layers.weight_init import trunc_normal_
from .utils.mask_embeeding import MaskEmbeeding, UnMaskEmbeeding_spa, UnMaskEmbeeding_chan
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.vit_type == "decoder":
            self.unmask_embed_spa = UnMaskEmbeeding_spa(
                img_size, embed_dim, in_chans, patch_size, num_patches_spa, args
            )
            self.unmask_embed_chan = UnMaskEmbeeding_chan(
                img_size, embed_dim, in_chans, patch_size, num_patches_chan, args
            )
