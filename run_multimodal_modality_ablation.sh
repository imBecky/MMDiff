...
PREFIX="${MMDIFF_EXPERIMENT_TAG_PREFIX:-MODABL}"
# 与 param.py: TB_LOG_ROOT = Path('../../tf-logs') 对齐（在仓库根目录执行时）
TB_LOG_ROOT="${TB_LOG_ROOT:-../../tf-logs}"
CONSOLE_DIR="${TB_LOG_ROOT}/ablation_batch_console"
mkdir -p "$CONSOLE_DIR"
...
for combo in "${COMBOS[@]}"; do
  ...
  logfile="${CONSOLE_DIR}/${PREFIX}_${safe}.log"

  echo "---------- combo=${combo} | tag=${MMDIFF_EXPERIMENT_TAG} | console_tee=${logfile} ----------"
